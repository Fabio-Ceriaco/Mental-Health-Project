...
export default class NewEvaluation implements OnInit {
  ...
  //------------  Lifecycle Hooks------------//
  ngOnInit(): void {
    console.log('Initializing NewEvaluation component');
    this.employeeId = this.loginService.employee_Id()!; // Non-null assertion
    console.log('Logged in employee ID:', this.loginService.employee_Id()); // Debug log
    const questions = this.route.snapshot.data['questions'] ?? []; // Get resolved questions
    if (!questions.length) {
      console.warn('No questions found in resolver data');
    }
  ...
  splitIntoPages(list: any[], size: number): any[][] {
    ...
    console.log('Split into pages:', pages);
    return pages;
  }
  //------------ Navigation Methods ------------//
  nextPage() {
